...
let quoteRegex = /{quote:([0-9]+), source:([0-9]+)}/gi;
...
client.on("message", (message) => {
  if (message.author.id === client.user.id) //don't worry about type differences, just check that ids are equal
  {
    ...
    //let channel = message.channel; //(+quotes[1]).toString(2);
    let sourceChannel = client.channels.find(function (seeked) {
      return seeked.id == quotes[1]
    });
      ...
      sourceChannel.fetchMessages({
        ...
        for (let messageId of quotes) {
          if (messages.has(messageId)) {
            ...
            content.replace(new RegExp(`{quote:${messageId}, source:${sourceChannel}}`, "i"), "");
          }
        }

        if (quotedMessages.length >= 1) //don't do anything if there are no messages that can be successfully quoted
        {
          let splitMessage = content.split(/{quote:([0-9]+), source:([0-9]+)}/gi); //split into chunks of text without quotes
            ...
            if (quotedMessage) {
              channel.sendMessage(chunk, {
                embed: {
                  color: 0x913D88,
                  author: {
                    name: `${quotedMessage.author.username} a dit:`,
                    icon_url: quotedMessage.author.avatarURL ? quotedMessage.author.avatarURL : undefined
                  },
                  description: quotedMessage.content,
                  footer: {
                    text: 'In #' + quotedMessage.channel.name + ' from ' + quotedMessage.channel.guild.name + ' server.'
                  }
                }
              });
            } else if (chunk !== "" && chunk !== quotes[0] && chunk !== quotes[1]) {
              channel.sendMessage(chunk);
            }
          }
        }
